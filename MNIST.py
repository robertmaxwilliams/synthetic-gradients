...
optimizer = tf.train.GradientDescentOptimizer(0.05, name='gradients_descent_optimizer')
...
grad = grads_and_vars[2][0]#tf.gradients(cost, [w2])[0]
...
with tf.Session() as sess:
	...
	for i in range(100):
		for start, end in zip(range(0, len(trX), batch_size), range(batch_size, len(trX)+1, batch_size)):
			...
			print(i, np.mean(np.argmax(teY, axis=1) == predictions), synthgrad_cost, sep='\t')
